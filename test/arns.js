...
function invalidVariants([string]) {
  return string.split(/\:|\//)
    .map(substr => (
      invalidCharacters.map(char => string.replace(substr, char))
    ))
    .reduce((carry, arr) => (carry.push(...arr), carry), [])
}

const invalidArns = [
  'totally invalid arns, no sense was made here',
  ...invalidVariants `arn:partition:service:region:tenant:resource`,
  ...invalidVariants `arn:partition:service:region:tenant:restype/resource`,
  ...invalidVariants `arn:partition:service:region:tenant:restype:resource`,
  // Not allowing whitespace in resource name
...
export {validArns, invalidArns, validArnExpressions}
